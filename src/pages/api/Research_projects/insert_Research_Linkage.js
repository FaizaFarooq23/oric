...
export default async function handler(req, res) {
    const { username, Type_of_Linkage , Feild_of_Study, Nationality,Name_of_Research_Grant, Date_of_Agreement, Name_of_Host_Institute, Address_of_Host_Institute, Collaborating_Agency, Collaborating_Agency_Address,Scope,Features } = req.body;
    try {
        const research_linkage = await prisma.Research_Linkage.create({
            data: {
                ...
                Date_of_Agreement: Date_of_Agreement.toISOString,
        ...
        console.error(error);
        res.status(400).json({ message: "Error occurred while creating policy casestudy" });
    }
}
